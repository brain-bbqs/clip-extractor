...
- `?test&mock_video&mock_ready` now marks a real mid-clip selection (frame 12, or frames 6–21 of the mock video's 30) and types a description on its own, so the link lands directly on a saveable state instead of needing that done by hand before Save/Upload output can be previewed; `&frame=<n>`/`&snippet=<lo>-<hi>` name other indices ([#41](https://github.com/brain-bbqs/clip-extractor/pull/41))
...
- The saved bundle is now named `sub-<label>[_ses-<label>]_desc-extracted+clip.tar.gz`, replacing the old `desc-frame`/`desc-snippet` and dropping the `_bundle` suffix ([#41](https://github.com/brain-bbqs/clip-extractor/pull/41))
- Each delivery's derivatives now sit in a directory of their own (`derivatives/clip-extractor/sub-.../beh/<date-..._time-...|recording-...>/`), so a listing of `beh/` reads as one group per delivery; every file inside also carries that same entity, so it stays self-describing away from the directory, and carries no `desc-` of its own (only the pose overlay keeps `desc-overlay`) ([#41](https://github.com/brain-bbqs/clip-extractor/pull/41))
