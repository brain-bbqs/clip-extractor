...
- A new **Browse EMBER or DANDI** source pane finds an existing public video in either archive and streams it, so a clip can be pulled out of an archived recording without first knowing its URL ([#26](https://github.com/brain-bbqs/clip-extractor/pull/26))
- The browser reads the archives' public S3 manifests rather than their APIs, so listing datasets and their videos needs no sign-in and no token ([#26](https://github.com/brain-bbqs/clip-extractor/pull/26))
- EMBER opens showing only the datasets that actually hold video, with a switch to see the rest; DANDI, whose file lists are far too large to read wholesale, reads a dataset's when you open it and says beforehand what that will cost ([#26](https://github.com/brain-bbqs/clip-extractor/pull/26))
- The filter box matches a dataset by its number, its title or the name of a file inside it, and titles are cached between visits so it works straight away on a return ([#26](https://github.com/brain-bbqs/clip-extractor/pull/26))
- A video chosen from an archive records the archive's own asset URL as its source, rather than the content-addressed bucket URL its frames are actually read from ([#26](https://github.com/brain-bbqs/clip-extractor/pull/26))
...
- Added `src/lib/archives.ts` (the bucket listing, dataset index and video assets) and `src/lib/archiveNames.ts` (the cached dataset titles), with unit and integration coverage ([#26](https://github.com/brain-bbqs/clip-extractor/pull/26))
