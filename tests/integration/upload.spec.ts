...
// upload's own instant, so only its shape (17 digits) is pinned down here; it names both this
// delivery's own directory under derivatives/ and every filename inside it (see lib/bidsPath.ts's
// derivativesDirectory), which the backreference below holds to the same value.
