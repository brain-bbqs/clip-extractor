...
- **Two-handle trim track** — a second track under the playhead slider carrying the **In** and **Out** markers and the band between them, so scrubbing and trimming never compete for the same drag. Each marker tapers to a point at the bottom, so its tip names one frame on the ruler rather than covering a dozen of them. Drag either marker to trim, drag the band to slide the whole range at its current length, or press anywhere on the track to bring the nearer marker to it. Markers are keyboard-operable (`←/→` by one frame, `Shift` by ten, `Home`/`End` to the bounds), and `[` / `]` (or `I` / `O`) still mark either end at the playhead.
- **Time ruler** — hash marks under the timeline, subdivided and labelled from the loaded video's own duration (aiming for roughly six labelled divisions, whether the clip runs seconds or an hour), so a selection reads as a stretch of time and not only as two frame indices. It sits under the playhead in both modes.
