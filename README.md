...
- **Optional SLEAP annotations step** — an on/off switch at the top right of the load card (default off) reveals a second card for loading a `.slp` (`loadSlp({ openVideos: false })`); dropping a `.slp` anywhere enables it automatically.
  - **The `.slp` has to describe the loaded video.** A `.slp` records the video it was labeled against, so the card compares the two on load — in either order, since the video can be swapped underneath a loaded `.slp` — and refuses a file whose video name, frame count, frame size, or labeled frame range does not fit, naming each field that disagrees and asking for another file. Nothing is kept from a refused file: no overlay, no annotations in a delivery. The name is compared without its directory or extension, so a re-encoded `mice.avi` still pairs with `mice.mp4`; it is checked at all because the `.slp` format records no checksum of the video, and a file that stored no shape has nothing else to identify it by. A differing fps is only logged to the console, since it cannot put a pose on the wrong frame.
